...
class Pasante inherits Cargo {
    ...
    override method sueldoBaseMensual(persona) = self.sueldoPorHora(persona) * persona.horasPorDia() * (22 - diasDeEstudio)
}
...
class VicepresidenteJunior inherits Gerente {
    ...
    override method sueldoPorHora(persona) = super(persona) + 1.03
}
...
class Sucursal { 
    ...
    method transferir(empleado, otraSucursal) {
        if(otraSucursal.esViable() && self.cantidadDeEmpleados() >= 3) {
            ...
            empleado.cambiarDeSucursal(otraSucursal) 
        }
        else {
...
class Compleja {
    ...
    method motivacion(persona) = self.sumaDePersonalidades() / self.cantidadDePersonalidades()
